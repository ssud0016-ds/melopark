...
│   ├── App.tsx                  # Root providers, theme, fonts, Mapbox init
│   ├── app.config.js            # Expo Android config (`app.melopark`, min SDK 26)
│   ├── eas.json                 # EAS APK build profiles
│   ├── src/navigation/          # Native stack + bottom tabs + deep linking
│   ├── src/screens/             # Map, predictions, about, terms, attribution
│   ├── src/components/          # Mapbox map, sheets, chrome, forecast UI
│   ├── src/hooks/               # Data polling, filters, destination, preferences
│   ├── src/services/            # Mobile API clients for backend endpoints
│   └── .env.example
├── backend/
...
The Android client lives in `mobile/` and is an Expo-managed React Native app.
There is no committed native `android/` project; native Android files are
generated by Expo prebuild/EAS when needed.

Android app details:

- App version: `1.0.0`
- Android package: `app.melopark`
- Minimum Android SDK: `26`
- Platform target in Expo config: Android only (`platforms: ["android"]`)
- Runtime stack: Expo SDK 55, React Native 0.83.6, React 19.2, TypeScript
- Production map engine: `@rnmapbox/maps`
- Deep links: `melopark://` and App Links for `https://melopark.app`

### Android UI composition

- `mobile/index.js` registers the Expo root component.
- `mobile/App.tsx` loads fonts/theme, configures Mapbox, and wraps global
  providers for gestures, safe areas, toast, filters, destination state, map
  fly targets, navigation, and offline status.
- `mobile/src/navigation/RootNavigator.tsx` defines the native stack, with
  `BottomTabs` for the Map and Predictions tabs.
- `mobile/src/screens/MapScreen.tsx` is the primary app surface. It composes
  the Mapbox map, pressure layers, search chrome, onboarding/help modals, and
  bottom sheets for bay, segment, filter, settings, and parking chance details.
- `mobile/src/screens/PredictionsScreen.tsx` presents CBD forecast and
  alternative parking pressure data.

### Android data flow

- Live bays: `useBays` -> `fetchParkingBays()` -> `GET /api/parking`
- Bay legality: `apiBays.ts` -> `/api/bays/{id}/evaluate` and
  `/api/bays/evaluate-bulk`
- Pressure data: `useBusyNow` / pressure hooks -> `apiPressure.ts` ->
  `/api/pressure/*`
- Forecasts: `useParkingForecast` -> `apiForecasts.ts` -> `/api/forecasts/*`
- Preferences and onboarding state use AsyncStorage on device.
...
Required for a fully functional mobile build:

- `EXPO_PUBLIC_API_URL` — backend API base URL used by mobile services
- `MAPBOX_PUBLIC_TOKEN` — runtime Mapbox token for the production map
- `MAPBOX_DOWNLOAD_TOKEN` — build-time token for the Mapbox native SDK

Optional:

- `GOOGLE_MAPS_ANDROID_API_KEY` — only needed for the optional
  `react-native-maps` spike, not the production Mapbox map.
...
### 3) Start Android app (new terminal)
